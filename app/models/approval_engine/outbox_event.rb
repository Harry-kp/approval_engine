module ApprovalEngine
  ...
  class OutboxEvent < ApplicationRecord
    ...
    # never a host record — which is why record_id is typed uuid. The column is
    # NOT NULL: every event is created *with* a record. `optional: true` is not a
    # contradiction — it only relaxes the presence check at load/update time, so
    # an event whose record was destroyed before relay (host record deleted → the
    # engine rows cascade away) can still be retired (marked processed) instead of
    # poisoning the queue.
